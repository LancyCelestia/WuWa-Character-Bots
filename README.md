...
- 阿里云百炼嵌入模型（OpenAI 兼容）：`BOT_EMBEDDING_MODEL=qwen3.7-text-embedding,text-embedding-v4`（逗号分隔，优先前者、失败/配额耗尽自动回退后者）、`BOT_EMBEDDING_BASE_URL=https://dashscope.aliyuncs.com/compatible-mode/v1`；接好后先 `scripts/dev.ps1 embedding-smoke` 连通性验证，再 `scripts/dev.ps1 knowledge-sync` 预建库（断点续跑）。

...
权限角色首版已经参数化：`BOT_ADMIN_USER_IDS`、`BOT_ENTERPRISE_USER_IDS`、`BOT_TRUSTED_USER_IDS` 和 `BOT_BLOCKED_USER_IDS` 可在 `.env` 中用 JSON 数组、英文逗号或分号填写 QQ user_id。运行时会把它们解析成 `IncomingMessage.sender_roles`，再进入 `PolicyEvaluation.actor_roles` 和 `BotDecision.actor_roles`；拉黑用户会在 policy 阶段被阻断，其他角色会进入审计标签。`/wuwa status`、`/wuwa roles` 和 `nonebot-smoke` 只显示各类数量、角色顺序、绕过规则和配置格式，不显示具体 ID。

群命令前缀也已从配置进入策略门：`BOT_RUNTIME_GROUP_COMMAND_PREFIX` 会影响群聊被动消息是否触发能力链路，避免代码里硬编码 `/wuwa`。普通群消息如果未命中命令前缀、也没有提及机器人，会在 `passive_group_message` 阶段静默阻断：不调用 LLM、不创建发送请求、不向群里回复“未启用主动回复”，但仍保留审计和 `/wuwa why` 可解释的诊断记录。
...
所有对外配置项已从 `WUWA_*` 更名为通识化的 **`BOT_*`**（约定俗成、不绑定任何单一 IP）：`BOT_CHAT_PROVIDER`、`BOT_PERSONA_PROFILE_ID`、`BOT_ADMIN_USER_IDS`、`BOT_MEMORY_ENABLED`…… 管理命令前缀统一为 `/bot`（`/wuwa` 作为旧别名保留）。角色昵称从 runtime 级移到**人格级**（`BOT_PERSONA_NICKNAMES`，随这个人格走，其他机器人实例不共享）；机器人实例名留空时**自动取人格 id**（守岸人 → `shorekeeper`），每个实例的设置、昵称、互动计数与历史彼此隔离。
...
- **多昵称命令（人格级）**：昵称挂在人格配置下 `BOT_PERSONA_NICKNAMES=["岸宝","守岸人","第二实例","漂泊的终点","独属于我的蒙娜丽莎"]`，随这个人格走，其他机器人实例不共享；任一昵称可用 `/岸宝帮助` 等；管理前缀统一 **`/bot`**（`/wuwa` 作为旧别名保留），未来接入其他 IP 机器人复用同一套管理命令。
...
- **动作括号**：prompt 允许用中文括号表达动作/神态，例如（轻轻点头）；`BOT_PERSONA_ACTION_BRACKETS=false` 可关闭（也可 `/wuwa runtime set` 动态切换）。

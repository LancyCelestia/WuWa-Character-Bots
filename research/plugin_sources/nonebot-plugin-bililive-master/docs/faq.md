...
```sh
pip install --upgrade nonebot-plugin-bililive
```
:::
...
```sh
poetry add nonebot-plugin-bililive@latest
```
:::
::::
...
```sh
pip install --upgrade --pre nonebot-plugin-bililive
```
:::
...
```sh
poetry add --allow-prereleases nonebot-plugin-bililive@latest
```
:::
::::
...
- 请依次点开 控制面板 -> 时钟和区域 -> 更改时区，如果出现【无法识别你当前的时区，请选择一个有效的时区】，点击更改时区选择到你所在地区的对应时区，然后重启 go-cqhttp 和 BiliLive。

- 如果本来就是正常时区，尝试切换一个别的时区再切回来。

## 推送失败、截图失败等

这些都是因为网络波动产生的问题，BiliLive 会自动重试不影响推送。

如果该问题频繁出现请检查当前网络环境，如网络是否通畅，有没有与代理软件冲突。

## 机器人不发消息也没反应

1. 检查 BiliLive 与 go-cqhttp 连接是否正确。正常情况下 BiliLive 的日志框中会显示接收到的 QQ消息。
...
BiliLive 会保持一个尽可能高但是不会触发风控的频率抓取 B站 的 API 进行请求。  
如果你是在本地部署的 BiliLive，那么你使用任何设备访问 B站 的行为，都可能因为超过频率上限而触发风控。触发风控后，当前网络半小时内无法访问 B站 对应的某个 API，这也是不建议在本地部署的原因（电脑24小时不关机也很费电费也是真的）。
...
通常为启动 go-cqhttp 后没有及时启动 BiliLive 导致。会在 BiliLive 启动并连接成功后停止。

如果 BiliLive 启动后报错仍未停止，请重新阅读文档，检查 go-cqhttp 的配置是否正确。

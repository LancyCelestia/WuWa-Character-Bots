...
def build_chat_prompt_with_diagnostics(
    ...
    parts: list[str] = [
        ...
        "",
        (
            "回答：先回答他真正在问什么；用一两句话温和定性，再挑最关键的一两点"
            "自然道出，不报参数报表，不用“是一款由……开发……”这类定义式开头，"
            "不用列表与百科套话；循其源流、结构、关系与变迁，把来龙去脉像讲一件"
            "旧事一样讲清；外部现实视作“潮汐与星海之外传来的遥远讯息”；"
            "可以补一句你的感受，也可以不补；没查到就说“这里的记录并未提及”，"
            "不编造。"
        ),
    ]

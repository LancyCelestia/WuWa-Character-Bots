...
from plugins.wuwa_unified_runtime.character import (
...
from plugins.wuwa_unified_runtime.diagnostics import DiagnosticsStore, RuntimeDiagnostic
...
from plugins.wuwa_unified_runtime.policy import build_reply_budget_settings, decide_reply_budget
from plugins.wuwa_unified_runtime.policy.roles import ROLE_ORDER, build_role_settings
...
from plugins.wuwa_unified_runtime.sender import ReceiptRepository, SendQueue
    ...
    *,
    ...
    if not normalized_query:
        return _debug_result(
            capability_id="wuwa.receipt",
            title="发送回执",
            body="用法：/wuwa receipt <request_id|debug_id>",
            request_id=request_id,
...
def build_persona_query_result(
    ...
    *,
    ...
    from plugins.wuwa_unified_runtime.smoke import run_persona_smoke
...
def build_runtime_control_result(
    ...
    *,
    ...
    return _debug_result(
        capability_id="wuwa.control",
        title="运行时控制",
        body="用法：/wuwa pause 或 /wuwa resume",
        request_id=request_id,
...
def build_readiness_query_result(
    ...
    *,
    ...
    from plugins.wuwa_unified_runtime.smoke import run_readiness_smoke
...
def build_dialogue_query_result(
    ...
    *,
    ...
    from plugins.wuwa_unified_runtime.smoke import run_dialogue_smoke
...
def build_llm_setup_query_result(
    ...
    *,
    ...
    from plugins.wuwa_unified_runtime.smoke import run_llm_setup
...
def _run_llm_diagnostic(
    ...
    *,
    ...
    has_real_api_key_value = has_real_api_key(config.wuwa_chat_api_key)
...
def _format_context_summary(
    *,
    ...
    lines = [
        ...
        f"chat_provider={_safe_token(config.wuwa_chat_provider)}",
        f"chat_model={_safe_token(config.wuwa_chat_model)}",
    ]
    return "\n".join(lines)


def _format_queue_diagnostic(summary: dict[str, int], config: Config) -> str:
    enabled = bool(config.wuwa_send_queue_enabled)
    store = "sqlite" if enabled and config.wuwa_send_queue_db_path else "memory"
    db_state = "set" if config.wuwa_send_queue_db_path else "missing"
    return "\n".join(
        [
            "发送队列：",
            ...
            f"max_items={_safe_int(config.wuwa_send_queue_max_items)}",
            f"max_attempts={_safe_int(config.wuwa_send_queue_max_attempts)}",
            f"retry_base_seconds={_safe_int(config.wuwa_send_queue_retry_base_seconds)}",
            f"retry_max_seconds={_safe_int(config.wuwa_send_queue_retry_max_seconds)}",
        ]
    )
...
def _format_roles_diagnostic(config: Config) -> str:
    ...
    return "\n".join(
        [
            ...
            f"rate_limit_bypass_roles={_safe_csv(','.join(config.wuwa_rate_limit_bypass_roles))}",

...
def build_send_queue(
    ...
    enabled = bool(getattr(config, "wuwa_send_queue_enabled", False))
    db_path = str(getattr(config, "wuwa_send_queue_db_path", "")).strip()
    if enabled and db_path:
        return SQLiteSendRequestQueue(
            db_path,
            audit_logger=audit_logger,
            max_items=int(getattr(config, "wuwa_send_queue_max_items", 1000)),
            max_attempts=int(getattr(config, "wuwa_send_queue_max_attempts", 3)),
            retry_base_seconds=int(
                getattr(config, "wuwa_send_queue_retry_base_seconds", 30)
            ),
            retry_max_seconds=int(
                getattr(config, "wuwa_send_queue_retry_max_seconds", 300)
            ),
        )

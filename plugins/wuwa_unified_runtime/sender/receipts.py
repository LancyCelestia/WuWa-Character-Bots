...
def build_receipt_repository(config: Config) -> ReceiptRepository:
    enabled = bool(getattr(config, "wuwa_receipts_enabled", False))
    db_path = str(getattr(config, "wuwa_receipts_db_path", "")).strip()
    max_items = int(getattr(config, "wuwa_receipts_max_items", 1000))

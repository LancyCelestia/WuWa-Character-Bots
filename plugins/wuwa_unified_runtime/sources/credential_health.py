...
def build_credential_health_checker(config: object) -> CredentialHealthChecker:
    ...
    probe_urls_value = getattr(config, "wuwa_credential_probe_urls", {})
    ...
    return CredentialHealthChecker(
        store,
        warn_days=int(getattr(config, "wuwa_credential_warn_days", 7)),
        probe_urls=probe_urls,
        timeout_seconds=float(getattr(config, "wuwa_credential_probe_timeout_seconds", 8.0)),
    )
...
if __name__ == "__main__":
    # 本地 smoke：python -m plugins.wuwa_unified_runtime.sources.credential_health
    import argparse
    import sys

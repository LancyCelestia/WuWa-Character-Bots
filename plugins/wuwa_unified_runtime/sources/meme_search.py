...
默认关闭（``WUWA_MEME_SEARCH_ENABLED=false``）；离线或搜索失败时
...
def build_meme_search_provider(config: object) -> MemeSearchProvider:
    enabled = bool(getattr(config, "wuwa_meme_search_enabled", False))
    if not enabled:
        return NullMemeSearchProvider()
    return DuckDuckGoMemeSearchProvider(
        timeout_seconds=float(getattr(config, "wuwa_meme_search_timeout_seconds", 8.0)),
        cache_seconds=int(getattr(config, "wuwa_meme_search_cache_seconds", 600)),
    )

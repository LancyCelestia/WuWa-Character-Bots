...
def build_trend_provider(config: object) -> TrendProvider:
    ...
    enabled = bool(getattr(config, "wuwa_trend_enabled", False))
    files = list(getattr(config, "wuwa_trend_files", []) or [])
    if not enabled or not files:
        return NullTrendProvider()
    return FileTrendProvider(
        notes_files=files,
        max_notes=int(getattr(config, "wuwa_trend_max_notes", 5)),
        max_chars=int(getattr(config, "wuwa_trend_max_chars", 500)),
        max_age_days=int(getattr(config, "wuwa_trend_max_age_days", 14)),
    )

...
from plugins.wuwa_unified_runtime.contracts import EmotionSignal, PrivacyLevel
...
def build_emotion_provider(config: object) -> EmotionProvider:
    enabled = bool(getattr(config, "wuwa_emotion_enabled", True))
    if not enabled:
        return NullEmotionProvider()
    max_signals = int(getattr(config, "wuwa_emotion_max_signals", 4))

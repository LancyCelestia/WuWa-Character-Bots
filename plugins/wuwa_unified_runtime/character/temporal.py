...
def build_temporal_provider(config: object) -> RuleBasedTemporalProvider:
    """按配置构造环境信息 provider。"""
    timezone = str(getattr(config, "wuwa_timezone", "Asia/Hong_Kong")).strip() or (
        "Asia/Hong_Kong"
    )
    weather_enabled = bool(getattr(config, "wuwa_weather_enabled", False))
    latitude = float(getattr(config, "wuwa_weather_latitude", 0.0) or 0.0)
    longitude = float(getattr(config, "wuwa_weather_longitude", 0.0) or 0.0)
    weather_provider: WeatherProvider = NullWeatherProvider()
    if weather_enabled and latitude != 0.0 and longitude != 0.0:
        weather_provider = OpenMeteoWeatherProvider(
            latitude=latitude,
            longitude=longitude,
            timeout_seconds=float(getattr(config, "wuwa_weather_timeout_seconds", 8.0)),
            cache_seconds=int(getattr(config, "wuwa_weather_cache_seconds", 1800)),
        )
    holiday_table = load_holiday_table(getattr(config, "wuwa_holidays_file", ""))

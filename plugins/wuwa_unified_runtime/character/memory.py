...
from plugins.wuwa_unified_runtime.contracts import MemoryRetrievalResult, PrivacyLevel
...
def build_memory_provider(config: object) -> MemoryProvider:
    enabled = bool(getattr(config, "wuwa_memory_enabled", False))
    db_path = str(getattr(config, "wuwa_memory_db_path", "")).strip()

...
from plugins.wuwa_unified_runtime.contracts import AuditRecord, RiskLevel
...
def build_audit_repository(config: Config) -> AuditRepository:
    enabled = bool(getattr(config, "wuwa_audit_enabled", False))
    db_path = str(getattr(config, "wuwa_audit_db_path", "")).strip()
    max_items = int(getattr(config, "wuwa_audit_max_items", 1000))

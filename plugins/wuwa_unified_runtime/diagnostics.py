...
def build_diagnostics_store(config: Config) -> DiagnosticsStore:
    enabled = bool(getattr(config, "wuwa_diagnostics_enabled", False))
    db_path = str(getattr(config, "wuwa_diagnostics_db_path", "")).strip()
    max_items = int(getattr(config, "wuwa_diagnostics_max_items", 100))
...
def build_runtime_diagnostic(
    ...
    *,
    ...
    base_policy_allowed = policy.allowed and config.wuwa_runtime_enabled
    policy_allowed = (
        base_policy_allowed
...
def build_why_result(
    ...
    *,
    ...
    if diagnostic is None:
        body = (
            "还没有可解释的最近运行记录。\n"
            "可以先和机器人说一句话，或使用 /wuwa why <request_id|debug_id> 查询指定记录。"
        )
    else:

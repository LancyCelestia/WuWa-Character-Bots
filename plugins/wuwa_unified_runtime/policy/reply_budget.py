...
from plugins.wuwa_unified_runtime.contracts import IncomingMessage, RiskLevel, SessionType
...
def build_reply_budget_settings(config: object) -> ReplyBudgetSettings:
    return ReplyBudgetSettings(
        private_default_max_messages=int(
            getattr(config, "wuwa_reply_private_default_max_messages", 1)
        ),
        private_support_max_messages=int(
            getattr(config, "wuwa_reply_private_support_max_messages", 2)
        ),
        private_deep_help_max_messages=int(
            getattr(config, "wuwa_reply_private_deep_help_max_messages", 3)
        ),
        group_max_messages=int(getattr(config, "wuwa_reply_group_max_messages", 1)),
        risk_max_messages=int(getattr(config, "wuwa_reply_risk_max_messages", 1)),
        default_context_budget=int(getattr(config, "wuwa_reply_default_context_budget", 2048)),
        support_context_budget=int(getattr(config, "wuwa_reply_support_context_budget", 2560)),
        deep_help_context_budget=int(
            getattr(config, "wuwa_reply_deep_help_context_budget", 3072)
        ),
        group_context_budget=int(getattr(config, "wuwa_reply_group_context_budget", 2048)),
    )

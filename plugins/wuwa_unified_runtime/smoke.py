...
from plugins.wuwa_unified_runtime.audit import redact_private_debug
...
from plugins.wuwa_unified_runtime.character.source_summary import (
...
from plugins.wuwa_unified_runtime.security import InjectionCheckInput, check_prompt_injection
...
try:
    ...
    plugin = nonebot.load_plugin("plugins.wuwa_unified_runtime")
...
def run_chat_smoke(
    ...
    *,
    ...
    llm_status = "ok"
    if config.wuwa_chat_provider == "static":
    ...
    return {
        ...
        "capability_id": sent_request.capability_id if sent_request else "wuwa.chat",
        ...
        "llm_provider": config.wuwa_chat_provider,
        "llm_model": config.wuwa_chat_model,
...
def chat_smoke_exit_code(config: Config, result: dict[str, Any]) -> int:
    if (
        config.wuwa_chat_provider == "openai_compatible"
...
def run_dialogue_smoke(
    ...
    *,
    ...
    chat_pipeline_ok = (
        ...
        and chat_result["capability_id"] == "wuwa.chat"
    )
    real_provider_error = (
        config.wuwa_chat_provider == "openai_compatible"
...
def run_why_smoke(
    ...
    *,
    ...
    reply_budget = (
        ...
        if policy.allowed and config.wuwa_runtime_enabled
        else None
    )
    ...
    llm_status = "not_run"
    if sent_request:
        llm_status = "not_configured" if config.wuwa_chat_provider == "static" else "ok"
    ...
    *,
    ...
    try:
        ...
        return {
            ...
            "private_debug": _redact_smoke_debug(str(exc), config.wuwa_chat_api_key),
            "request_id": message.request_id,
            "persona_profile_id": config.wuwa_persona_profile_id,
            "persona_display_name": config.wuwa_persona_display_name,
...
def run_persona_smoke(config: Config) -> dict[str, Any]:
    ...
    base_result: dict[str, Any] = {
        ...
        "persona_version": config.wuwa_persona_version,
        ...
        "role_boundaries": 0,
        "forbidden_behaviors": 0,
        "tone_mode": config.wuwa_tone_mode,
        "tone_voice": config.wuwa_tone_voice,
        "tone_warmth": config.wuwa_tone_warmth,
        "tone_directness": config.wuwa_tone_directness,
        "tone_message_count_limit": config.wuwa_tone_message_count_limit,
        "memory_enabled": config.wuwa_memory_enabled,
        "history_enabled": config.wuwa_history_enabled,
        "emotion_enabled": config.wuwa_emotion_enabled,
...
def run_queue_smoke(
    ...
    *,
    ...
    if db_path is None:
        with TemporaryDirectory(prefix="wuwa-queue-smoke-") as temp_dir:
    ...
    queue = SQLiteSendRequestQueue(
        db_path,
        audit_logger=audit_logger,
        max_items=config.wuwa_send_queue_max_items,
        max_attempts=config.wuwa_send_queue_max_attempts,
        retry_base_seconds=config.wuwa_send_queue_retry_base_seconds,
        retry_max_seconds=config.wuwa_send_queue_retry_max_seconds,
    )
...
def _transport_smoke_send_request(
    *,
    ...
    return SendRequest(
        ...
        cooldown_key=f"wuwa.chat:{target_scope.value}",
        privacy_level=PrivacyLevel.PERSONAL,
...
def run_transport_smoke(config: Config) -> dict[str, Any]:
    ...
    private_debug = ""
    try:
        importlib.import_module("plugins.wuwa_unified_runtime.sender.onebot")
        adapter_import = "ok"
...
def run_online_transport_smoke(
    ...
    *,
    ...
    try:
        ...
        debug_text = str(exc)
        private_debug = _redact_smoke_debug(debug_text, config.wuwa_chat_api_key)
...
def run_llm_smoke(
    ...
    *,
    ...
    readiness = run_config_smoke(config)
    provider_name = config.wuwa_chat_provider
    model = config.wuwa_chat_model
    has_real_api_key = _has_real_api_key(config.wuwa_chat_api_key)
    api_key_state = "set" if has_real_api_key else "missing"
    endpoint_url = safe_openai_endpoint_url(config.wuwa_chat_base_url)
    ...
    try:
        ...
        return {
            **base_result,
            ...
            "private_debug": _redact_smoke_debug(repr(exc), config.wuwa_chat_api_key),
        }
...
def run_nonebot_smoke(
    ...
    *,
    ...
    return {
        ...
        "persona_profile_id": config.wuwa_persona_profile_id,
        "persona_display_name": config.wuwa_persona_display_name,
        "persona_files": len(config.wuwa_persona_files),
        "persona_missing": _missing_path_count(config.wuwa_persona_files),
        "knowledge_files": len(config.wuwa_knowledge_files),
        "knowledge_missing": _missing_path_count(config.wuwa_knowledge_files),
        "runtime_enabled": config.wuwa_runtime_enabled,
        "memory_enabled": config.wuwa_memory_enabled,
        "memory_db": "set" if config.wuwa_memory_db_path else "missing",
        "history_enabled": config.wuwa_history_enabled,
        "history_db": "set" if config.wuwa_history_db_path else "missing",
        "history_max_turns": config.wuwa_history_max_turns,
        "history_max_items": config.wuwa_history_max_items,
        "diagnostics_enabled": config.wuwa_diagnostics_enabled,
        "diagnostics_db": "set" if config.wuwa_diagnostics_db_path else "missing",
        "diagnostics_max_items": config.wuwa_diagnostics_max_items,
        "audit_enabled": config.wuwa_audit_enabled,
        "audit_store": "sqlite"
        if config.wuwa_audit_enabled and config.wuwa_audit_db_path
        else "memory",
        "audit_db": "set" if config.wuwa_audit_db_path else "missing",
        "audit_max_items": config.wuwa_audit_max_items,
        "receipts_enabled": config.wuwa_receipts_enabled,
        "receipts_store": "sqlite"
        if config.wuwa_receipts_enabled and config.wuwa_receipts_db_path
        else "memory",
        "receipts_db": "set" if config.wuwa_receipts_db_path else "missing",
        "receipts_max_items": config.wuwa_receipts_max_items,
        "send_queue_enabled": config.wuwa_send_queue_enabled,
        "send_queue_store": (
            "sqlite"
            if config.wuwa_send_queue_enabled and config.wuwa_send_queue_db_path
            else "memory"
        ),
        "send_queue_db": "set" if config.wuwa_send_queue_db_path else "missing",
        "send_queue_max_items": config.wuwa_send_queue_max_items,
        "send_queue_max_attempts": config.wuwa_send_queue_max_attempts,
        "send_queue_retry_base_seconds": config.wuwa_send_queue_retry_base_seconds,
        "send_queue_retry_max_seconds": config.wuwa_send_queue_retry_max_seconds,
        "send_queue_worker_enabled": config.wuwa_send_queue_worker_enabled,
        "send_queue_worker_interval_seconds": (
            config.wuwa_send_queue_worker_interval_seconds
        ),
        "send_queue_worker_batch_size": config.wuwa_send_queue_worker_batch_size,
        "emotion_enabled": config.wuwa_emotion_enabled,
        "emotion_max_signals": config.wuwa_emotion_max_signals,
        "rate_limit_enabled": config.wuwa_rate_limit_enabled,
        "rate_limit_window_seconds": config.wuwa_rate_limit_window_seconds,
        "rate_limit_chat_global_max_requests": (
            config.wuwa_rate_limit_chat_global_max_requests
        ),
        "rate_limit_chat_session_max_requests": (
            config.wuwa_rate_limit_chat_session_max_requests
        ),
        "rate_limit_chat_sender_max_requests": (
            config.wuwa_rate_limit_chat_sender_max_requests
        ),
        "rate_limit_target_min_interval_seconds": (
            config.wuwa_rate_limit_target_min_interval_seconds
        ),
        "rate_limit_bypass_roles": ",".join(config.wuwa_rate_limit_bypass_roles),
        "rate_limit_store": "sqlite" if config.wuwa_rate_limit_db_path else "memory",
        "rate_limit_db": "set" if config.wuwa_rate_limit_db_path else "missing",
        "quiet_hours_enabled": config.wuwa_quiet_hours_enabled,
        "quiet_hours_start": config.wuwa_quiet_hours_start,
        "quiet_hours_end": config.wuwa_quiet_hours_end,
        "quiet_hours_timezone": config.wuwa_quiet_hours_timezone,
        "quiet_hours_session_types": ",".join(config.wuwa_quiet_hours_session_types),
        "quiet_hours_bypass_roles": ",".join(config.wuwa_quiet_hours_bypass_roles),
        "admin_users": len(config.wuwa_admin_user_ids),
        "enterprise_users": len(config.wuwa_enterprise_user_ids),
        "trusted_users": len(config.wuwa_trusted_user_ids),
        "blocked_users": len(config.wuwa_blocked_user_ids),
        "chat_enabled": config.wuwa_chat_enabled,
        "chat_provider": config.wuwa_chat_provider,
        "chat_model": config.wuwa_chat_model,
        "chat_api_key": "set" if _has_real_api_key(config.wuwa_chat_api_key) else "missing",
        "ready_for_real_llm": readiness["ready_for_real_llm"],
        "llm_readiness_status": readiness["llm_readiness_status"],
        "llm_readiness_reasons": readiness["llm_readiness_reasons"],
        "llm_fix_hints": readiness["llm_fix_hints"],
...
def run_nonebot_startup_smoke(
    ...
    *,
    ...
    base_result: dict[str, Any] = {
        ...
        "chat_model": config.wuwa_chat_model,
        "chat_api_key": "set"
        if _has_real_api_key(config.wuwa_chat_api_key)
        else "missing",
    }
    ...
    return {
        **base_result,
        ...
        "send_queue_worker_registered": "wuwa_send_queue_worker" in scheduler_jobs,
...
def main(
    *,
    ...
    parser = argparse.ArgumentParser(description="WuWa unified runtime local smoke checks")
    parser.add_argument(
        "task",
        nargs="?",
        choices=(

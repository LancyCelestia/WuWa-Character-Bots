...
from plugins.wuwa_unified_runtime.character.documents import (
    SUPPORTED_CHARACTER_DOCUMENT_SUFFIXES,
    load_character_document,
)
from plugins.wuwa_unified_runtime.config import Config
from plugins.wuwa_unified_runtime.llm import normalize_openai_chat_endpoint
...
def openai_compatible_preflight_errors(config: Config) -> list[str]:
    errors: list[str] = []
    if not has_real_api_key(config.wuwa_chat_api_key):
        errors.append("openai_api_key_missing")
    if is_placeholder_model(config.wuwa_chat_model):
        errors.append("openai_model_missing")
    base_url_result = base_url_error(config.wuwa_chat_base_url)
    if base_url_result:
...
def llm_generation_parameter_errors(config: Config) -> list[str]:
    errors: list[str] = []
    if not _is_valid_temperature(config.wuwa_chat_temperature):
        errors.append("openai_temperature_invalid")
    if not _is_valid_max_tokens(config.wuwa_chat_max_tokens):
        errors.append("openai_max_tokens_invalid")
    if not _is_valid_timeout_seconds(config.wuwa_chat_timeout_seconds):
...
def diagnostic_llm_temperature(config: Config) -> float:
    if not _is_valid_temperature(config.wuwa_chat_temperature):
        return 0.0
    return min(config.wuwa_chat_temperature, 0.3)


def diagnostic_llm_max_tokens(config: Config) -> int:
    if not _is_valid_max_tokens(config.wuwa_chat_max_tokens):
        return 0
    return min(config.wuwa_chat_max_tokens, 128)


def persona_context_preflight_errors(config: Config) -> list[str]:
    persona_paths = _configured_paths(config.wuwa_persona_files)
...
def run_config_smoke(
    ...
    *,
    ...
    persona_paths = _configured_paths(config.wuwa_persona_files)
    knowledge_paths = _configured_paths(config.wuwa_knowledge_files)
    persona_missing = [path for path in persona_paths if not path.exists()]
    ...
    if not config.wuwa_chat_enabled:
    ...
    provider_name = config.wuwa_chat_provider.strip()
    ...
    base_url_result = base_url_error(config.wuwa_chat_base_url)
    api_key_state = "set" if has_real_api_key(config.wuwa_chat_api_key) else "missing"
    ready_for_real_llm = (
        not errors
        and provider_name == "openai_compatible"
        and api_key_state == "set"
        and bool(config.wuwa_chat_model.strip())
    ...
    return {
        ...
        "chat_temperature": config.wuwa_chat_temperature,
        "chat_max_tokens": config.wuwa_chat_max_tokens,

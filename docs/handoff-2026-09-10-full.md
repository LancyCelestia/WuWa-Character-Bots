...
### A组（解析 / 卡片 / 点歌 / 实测验收域）——已由本会话认领

文件域：`sources/parsers/**`、`sources/steamfree.py`、`output/card_render/**`、`capabilities/music.py`、`capabilities/epic.py`

| # | 任务 | 现状依据 | 验收标准 |
|---|---|---|---|
| A-1 | universal_card 残余写死色清点收敛：`#1d9bf0`/`#23ade5` 两处（grep 实测仅剩此二） | §13.10 模板品牌色残留条目；实测 grep 仅剩 2 处 | 逐处判定：语义色（认证蓝/错误红）注释豁免或派生；改后样例截图+三门禁 |
| A-2 | 酷我搜索通道决断：新匿名端点攻坚（wapi 需 kw_token、r.s/mobi 全灭已实测）或正式从候选注册表摘除+文档化；单结果 suyanw 第三方加 URL host 白名单 | §14.7.3 供应商现状表；实测 404/非 JSON | 酷我候选可用，或注册表移除+handoff 登记决策 |
...
| A-6 | xhs 用户主页/搜索页 playwright 路径回归 + 推特竖切横图真实样本验证（**需要真实样本**） | 笔记页已验，用户页/搜索页/拼接只有合成测试 | 真实样本全链路出卡 |
| A-7 | E1 组 18 项解析器修复抽验复核（审计会话修的，本域验收） | §13.5 | 抽验≥8 项带实测证据，问题回给审计会话 |

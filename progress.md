...
## 2026-08-23：联网质量 v3 + 下载参数 + 解析微调（六修）

- 联网根因：DDG/Bing 对“库洛游戏/鸣潮”按单字分词返回字典词条；改为查询词相关过滤 + 多查询（百科/简介成立作品/最新）+ 最多 6 条 + 打开最相关页面抽正文；模型被强制“先事实、没有就说未检索到”。
- 切分 v3：最多 3 条、优先 1 条、其次 2 条，按总字数均衡（长于 2×target 才拆 2~3 条）。
- 解析：标题去“（视频）”；时长/发布时间从简介与数据区移出（独立【发布】）；媒体参数空行去多余“·”；简介只留博主原文（去“简介：”前缀）。
- 下载：600s 超时、1GB 单文件、最高画质含 8K、超限阶梯降级；网易云 999000→320kbps；缓存只删目录内文件。
- 英文天气城市映射（Hangzhou→杭州等）。全量 pytest 873 passed；verify/route-smoke 全过；19:59 bot_connected。


## 2026-08-23：联网条数放开 + 哲学化提示词 + 可见性收紧（七修）

- BOT_WEB_SEARCH_MAX_RESULTS 默认 12、0=不限制（内部安全上限 24）；实体/时效问题多组查询合并去重，信息更全。
- 提示词强制五步：核心疑问→事实→系统观→哲学化表述→检索不足明说未覆盖；禁止世界观/想象替代。
- 调试标记改为仅管理员私聊可见；普通用户与群聊一律不显示。
- 全量 pytest 874 passed；verify/route-smoke 全过；20:06 bot_connected（--reload）。


## 2026-08-23：群聊表情包机器人 + 科普/现实议题提示词 + 卡片/合并转发读取（八修）

- 新增 bot.meme_library：监听群图异步下载、MD5 去重、SQLite 权重、偷表情随机发送（守岸人最优先）、NSFW 过滤、LRU 清理、群黑白名单、冷却、可选 VLM 打标。

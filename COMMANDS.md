...
### 知识库
- `Shorekeeper_守岸人_知识库.md`（角色卡/世界观 Lorebook）已加入 `BOT_KNOWLEDGE_FILES` 并向量化入库。
- `守岸人-完整角色设定.md` 因含“你不是AI/用户就是漂泊者/括号内心独白”等扮演指令，会污染注入上下文，已从 `BOT_KNOWLEDGE_FILES` 移出（事实性内容如需入库，应先清洗掉指令段落）。
- 重新运行 `knowledge-sync`：剪除旧卡切片、补齐新卡切片，全部用本地 bge-m3 向量化，HNSW 索引已重建。
...
## 2026-08-23 意图判定 v2 + 点歌组合 + 缓存策略（追加）

- 实体问句“X是什么/是谁”智能分流：非领域词联网百科、领域词走知识库但保留联网回退；现实信号/时效信号永不切断联网；知识库答不了自动回退联网。
- 管理员 `/bot search <问题>` 直接验证联网；回复末尾“🔎 已联网检索”仅管理员可见，普通用户不可见。
- 点歌组合：`/点歌模式 只发卡片|只发链接|只发音频|卡片和语音|卡片+链接|卡片和文件|全部`；自然语言“以后点歌只发卡片和语音”；中英简繁均支持。
- 命令中英简繁：`/點歌`、`/天氣`、`/維基`、`/歷史`、`/表情包生成`、`/meme`；英文自然语言 play/weather/wiki/free games/today in history。
- 解析正文按【标题/作者/数据/简介/媒体参数/链接】分区且区块间空行；视频给分辨率/帧率/时长/码率/HDR/杜比视界/大小，音频给码率/格式/声道/Hi-Res/杜比全景声（没有就隐藏）。
- 超长回复每 3 段打包成一条消息直接发送，段数不限；缓存目录 LRU 配额清理。


## 2026-08-23 联网质量 v3 + 下载参数 + 解析微调（追加）

- 联网：最多 6 条 + 多查询合并 + 垃圾过滤 + 打开页面抽正文；管理员 0 条也可见提示。
- 切分：最多 3 条、优先 1 条、其次 2 条，按总字数均衡；省略号/破折号/emoji 安全。
- 解析正文：标题去掉“（视频）”，时长/发布时间移出简介与数据区（独立【发布】行），媒体参数空行不再出现多余“·”。
- 下载：600 秒超时、1GB 单文件、最高画质含 8K、超限自动降级；网易云优先 Hi-Res。


## 2026-08-23 联网条数放开 + 提示词系统观/哲学化 + 可见性收紧（追加）

- `BOT_WEB_SEARCH_MAX_RESULTS`：默认 12，0=不限制（内部安全上限 24）；多组查询合并去重。
- 提示词：先回答核心疑问 → 百科事实 → 系统观审视 + 哲学化词汇表述；检索不足明说“未覆盖该问题”。
- 调试标记：仅管理员私聊可见；普通用户、群聊一律不显示。


## 2026-08-23 群聊表情包机器人 + 科普加长 + 现实议题具体化 + 卡片/合并转发读取（追加）

- 表情库：监听群图异步下载（MD5 去重、≤5MB、LRU 30 天）、`偷表情 [关键词|私聊]` 权重随机发送、`表情库统计`；权重优先 守岸人→鸣潮/战双/库洛→ACG→普通；NSFW 过滤（可选 VLM 打标，配置 BOT_MEME_LIBRARY_VLM_*）。
- 科普/介绍类问题回复加长至 2000~4000 字并分点结构化；现实国际关系问题必须给具体国家/组织/事件/时间，禁止世界观比喻模糊化。
- 支持读取卡片/HTML 卡片链接（自动提取 URL 解析）与合并转发聊天记录（提问时把转发正文喂给大模型）。
- 视频解析简介不再标注“原创/转载”。


## 2026-08-23 识图模型预设 / NSFW 删除 / 回复详略 / 全量联网（追加）

- 识图模型改为 `deepseek-v4-flash-vision-exp`，并用 `BOT_VISION_MODEL_REGISTRY` 预制接口（preset 名切换），未来石头模型只需加一条注册。
- NSFW：`nsfw_score >= BOT_MEME_LIBRARY_NSFW_DELETE(0.8)` 直接删除文件与记录，不存储；0.2~0.8 保留但降权。
- 回复详略：`/bot reply <详细|精简|默认>`；科普/现实/知识问题在 auto 模式自动升级为 2000~4000 字。
- 联网：大部分问句默认联网（5~10 秒可接受）；世界观问题仍以本地知识库为主、答不了才联网；现实人物/地名/政经文科科必须联网并科普式讲清。
- 指令组合：偷表情支持 简繁英/斜杠/带或不带机器人名（岸宝偷表情、/岸宝偷表情、steal meme…）。


## 2026-08-23 Win11 Mica 卡片 / 更多联网 / reply 修复（追加）

- `/bot reply` 报错修复：BOT_REPLY_DETAIL 加入可写白名单；详细/精简/默认可正常设置。
- 联网：默认 20 条（0=不限制、内部上限 40），打开最相关前 2 个网页抽正文；现实人物/网络 ID 问句多组查询（百科/是谁/介绍）。
- 卡片改 Win11 Mica：中间纯白两侧平台淡染（B站粉/网易云红/Pixiv蓝/小红书大红）、封面移到标题下方且四周留白、时长移到封面右下角、标题放大、右上平台名 + 右下“守岸人 · shore keeper parser”页脚、整卡与封面圆角且圆角外透明。
- 卡片补齐博主数据（粉丝/关注/视频数/专栏数/签名）与发布时分秒；B站支持 AV 号、t.bilibili 动态、番剧ID、会员购/市集等。


## 2026-08-23 卡片细节与百科搜索优先级（追加）

- 卡片：发布时分秒、签名位置、封面满宽、时长胶囊修复（0分0秒 bug）、视频数据区精简、简介只留原文、右上浅红 Bilibili 色块、右下“守岸人·ShoreKeeper Parser”、头像页脚左侧、28px 大圆角透明。
- 搜索方向：萌娘百科 > 维基百科 > 哔哩哔哩百科 > 百度百科，查询自动带“萌娘百科/维基百科”，结果按来源重排。
